...
class SuiviLigne :
    ...
    def __init__(self):
        ...
        # Allouer de la memoire pour la conversion de la couleur
        # des pastilles de rgb en L*a*b* 
        self.lab_colors = np.zeros((len(self.colors), 1, 3), dtype="uint8")
        self.colorNames = []
        ...
        for (i, (name, rgb)) in enumerate(self.colors.items()):
            ...
            self.lab_colors = cv2.cvtColor(self.lab_colors, cv2.COLOR_RGB2LAB)
    ...
    def start_rover(self, q, q_past, q_cas) :

        q_cas.put(str(self.data).encode('utf-8'))
        q_past.put(str(self.mesure_pastilles).encode('utf-8'))

        while True:
            
            # Prises de frames
            self.counter +=1
            self.counter = self.counter % self.periode
            ...
            if len(contours) > 0:
                ...
                if instruction == "arret" 
                    self.bus.write_byte(self.address, 251)
                elif instruction == "ralentir" 
                    self.bus.write_byte(self.address, 252)
                else 
                    self.data = round(255*cx/656)
            ...
            if self.counter == 0 :
                ...
                # Appliquer un seuillage e l'image en niveau de gris
                ...
                # Intitialiser les tableaux donnant l'abscisse, l'ordonnee
                ...
                for c in cnts:
                    ...
                    # Si le nombre de pixels ne correspond pas e une
                    ...
                    if 300 < nb < 20000:
                        ...
                        # Reconverir en coordonnees vraies si l'image a ete
                        # reduite
                        c = c.astype("float")
                        c *= ratio
                        c = c.astype("int")
                        ...
                        # Mettre e jour les tableaux donnant l'abscisse,
                        # l'ordonnee et le nombre de pixels pour chacune
                ...
                # Une fois tous les contours analyses, proceder a
                # un moyennage si la meme couleur a ete detectee
                # plusieurs fois
                ...
                # Enregistrer l'image traitee dans un fichier
                # Sinon, communiquer

                self.mesure_pastilles = ''
                for i in range(len(self.colors)):
                    self.mesure_pastilles += str(xColors[i])+'\r\n'
                    self.mesure_pastilles += str(yColors[i])+'\r\n'
